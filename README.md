...
* Navigate into the cloned repository directory using the `cd aiml-genai-multimodal-agent/frontend/` command and run the command `pip install -r requirements.txt` to install the needed python libraries
...
* Open Streamlit_app.ipynb notebook. When the notebook starts, choose "Data Science 3.0" as image, "Python 3" as kernal and "" as instance type.
* Run the Streamlit_app.ipynb notebook.  This will generate the appropiate link to enter your Streamlit app from SageMaker studio. Click on the link to enter your Streamlit app. Happy querying :)
